...
def check_resources_for_coffee(type_of_coffee):
    """ This function checks, if resources are enough to prepare user selected coffee or not.
    If resources are enough it will call  buy_and_prepare_coffee function, if resources are not enough
     it will display which resource is not sufficient to prepare coffee"""
    for item in resources:
        if item in MENU[type_of_coffee]['ingredients']:
            if MENU[type_of_coffee]['ingredients'][item] > resources[item]:
                print(f"Sorry there is not enough '{item}' ")
                return
    buy_and_prepare_coffee(type_of_coffee)


def buy_and_prepare_coffee(type_of_coffee):
    """ This function requests user to insert money, If the money inserted is enough (or more than coffee price)
    then machine will proceed to prepare the coffee. If user inserted money is not sufficient for specific coffee then
     it will return the money and displays a message stating 'insufficient money' .
     If coffee is prepared, then this function will deduct the resources which are required for the specific coffee and
     it will update the resources set"""
    global coffee_price
