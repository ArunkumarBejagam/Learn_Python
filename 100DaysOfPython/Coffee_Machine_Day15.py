...
buy_coffee_from_machine = True

while buy_coffee_from_machine:
    choose_option = input("Select a Drink : 'espresso' ,'latte', 'cappuccino' :").lower()
    # quarter (.25), dime (.10), nickle (0.05), penny (0.01)
    # espresso 1.5, latte 2.5, cappuccino 3
    if choose_option == "report":
        print(f" Milk = {resources['milk']}ml \n Water = {resources['water']}ml \n Coffee = {resources['coffee']}gm ")
        print(f" Money = {money_earned_so_far}$ \n")
    elif choose_option in MENU:
        check_resources_for_coffee(choose_option)
    else:
