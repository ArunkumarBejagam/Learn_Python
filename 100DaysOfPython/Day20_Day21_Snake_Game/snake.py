...
class Snake:
    ...
    def create_snake(self):
        for i in range(SNAKE_SIZE):
            self.snakes_list.append(i)
            self.snakes_list[i] = Turtle(shape="square")
            self.snakes_list[i].color("white")
            self.snakes_list[i].penup()
            self.snakes_list[i].goto(x=self.x_axis, y=self.y_axis)
            self.x_axis -= 20
